...
| **`halftone`** | Retícula de impressão de quadrinhos e jornais antigos (`• ● ⬤`). |
| **`edge_art`** | Filtro de Sobel que transforma fotos em desenhos nanquim (`/ \ │ ─ +`). |
| **`glitch`** | Aberração cromática RGB horizontal com scanlines de fita VHS analógica. |
| **`pixel_mosaic`** | Sprites retrô quantizados para paletas clássicas (**PICO-8**, **Commodore 64**, **Game Boy**). |
| **`palette_swap`** | Quantiza para temas famosos de IDE: **Dracula**, **Catppuccin**, **Nord**, **Gruvbox**, **TokyoNight**. |
| **`rainbow_wave`** | Onda senoidal contínua de arco-íris animada sobre o texto. |
| **`chafa`** | Gráficos de alta densidade usando múltiplos blocos Unicode. |
| **`signature`** | Assinaturas e logotipos recortados cirurgicamente com moldura de terminal. |
...
| **`asciiquarium`**| Aquário dinâmico com peixes, tubarão e bolhas de ar. |
| **`donut_3d`** | O clássico Donut giratório de Andy Sloane com iluminação Phong e sombras. |
| **`synthwave_grid`**| Estrada synthwave dos anos 80 com sol neon e perspectiva infinita. |
| **`cava`** | Equalizador de áudio em tempo real com gradientes cyberpunk. |
| **`game_of_life`** | O Jogo da Vida de Conway com colônias celurares se multiplicando em loop. |
| **`pipes`** | Labirinto 3D de encanamentos procedurais coloridos. |
| **`cowsay`** | Balões de fala com mascotes ASCII (**Vaca**, **Dragão**, **Robô**, **Gato**, **Fantasma**). |
| **`tetris_reveal`**| Blocos de pixels caem do topo da tela com física até revelar a imagem. |
| **`ansi_cp437`** | Arte teletext de BBS dos anos 90 com os blocos clássicos do DOS (`░ ▒ ▓ █`). |
...
| **`tech_stack`** | Badges e arsenais de tecnologias em estilo neon dark. |
| **`ascii_diagram`**| Diagramas de arquitetura de software (Microsserviços, Event-Driven, Cloud Native). |
| **`tty_clock`** | Relógio digital LED estilo display 7-segmentos em tempo real. |
| **`neofetch`** | Painel de especificações do desenvolvedor estilo terminal macOS. |
| **`fortune_banner`**| Filosofia hacker e provérbios Unix com cursor animado. |

---

## 💻 Como Rodar em Outros Computadores (Guia Completo)

O projeto foi construído para ser **100% portátil** e rodar em qualquer máquina com **Python 3.10 ou superior** no Windows, Linux ou macOS.

### 📥 1. Clonar o Repositório
```bash
git clone https://github.com/ViniciusNoetzold/Mezzold-TermArt.git
cd Mezzold-TermArt
```
...
#### No Windows (1 Clique com os arquivos `.bat`):
- **`iniciar_studio.bat`**: Inicia o servidor, detecta o IP da sua rede local automaticamente e abre duas abas no navegador: uma para você (`localhost:7860`) e outra com o IP da rede para você enviar para outras pessoas da casa ou escritório!
- **`iniciar_tunel_publico.bat`**: Cria um túnel Cloudflare seguro instantâneo sem precisar abrir portas no roteador, gerando um link público (`https://....trycloudflare.com`) para qualquer um no mundo acessar o seu studio.
- **`iniciar_terminal.bat`**: Inicia o menu interativo no terminal de comando (CLI).
...
## 🧩 Como Criar e Adicionar Novos Motores

Adicionar um novo efeito à suíte requer apenas criar um arquivo em `src/termart/modules/<categoria>/`:
...
@registry.register
class MeuEfeito(BasePlugin):
    name = "meu_efeito"
    category = "fx"
    description = "Descrição do meu efeito incrível"
...
* **Criador & Arquiteto Líder:** [Vinícius Noetzold](https://github.com/ViniciusNoetzold) — **Mezzold Studios**
